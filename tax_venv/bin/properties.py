...
class CityProperty():
    ...
    def get_property_lots(self, property_data):
    ...
    def get_property_lots_blocks(self, *args):
        for property in args[0]:
            self.get_property_lots(property)


    def get_properties(self):
        for property_data in self.city_data:
            self.get_property_lots(property_data)


    def get_properties_threaded(self):
        threads = []
        max_threads = 169
        n1 = len(self.city_data)//max_threads
        while n1 == 0:
            max_threads = max_threads - 1
            n1 = len(self.city_data)//max_threads

        properties = [self.city_data[i:i+n1] for i in range(0, len(self.city_data), n1)]
        for i in range(0, max_threads):
            t = Thread(target=self.get_property_lots_blocks, args=(properties[i], ))
            t.start()
            threads.append(t)
...
if __name__ == "__main__":
    start = perf_counter()
    city_properties = CityProperty("0716")
    end = perf_counter()
